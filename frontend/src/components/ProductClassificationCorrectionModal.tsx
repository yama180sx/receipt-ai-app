...
import { AppButton, AppModal, AppSelect, AppTextInput } from './ui';
...
const scopeOptions: Array<{ value: ProductClassificationCorrectionScope; label: string; detail: string }> = [
  ...
  { value: 'same_classification_name', label: '同じ分類用名称にも適用', detail: '入力した分類用名称を世帯内で再利用します。' },
];

export const ProductClassificationCorrectionModal: React.FC<Props> = ({
  item, productTypes, selectedProductTypeId, scope, classificationName, loading, onClose,
  onProductTypeChange, onScopeChange, onClassificationNameChange, onSave,
}) => (
  <AppModal
    visible={Boolean(item)} onRequestClose={onClose} title="商品種別を修正"
    description={item ? `対象明細: ${item.name}` : undefined}
    footer={<>
      <AppButton title="キャンセル" variant="outline" onPress={onClose} disabled={loading} />
      <AppButton title="保存" onPress={onSave} loading={loading}
        disabled={!selectedProductTypeId || (scope === 'same_classification_name' && !classificationName.trim())} />
    </>}
  >
    <Text style={styles.label}>商品種別</Text>
    <AppSelect<number | null> selectedValue={selectedProductTypeId} onValueChange={onProductTypeChange}
    ...
    {scopeOptions.map((option) => {
      ...
      return <TouchableOpacity key={option.value} style={[styles.scopeOption, selected && styles.scopeOptionSelected]}
        onPress={() => onScopeChange(option.value)} accessibilityRole="radio" accessibilityState={{ selected }}>
    ...
    {scope === 'same_classification_name' ? <View style={styles.classificationNameField}>
      <Text style={styles.label}>分類用名称</Text>
      <AppTextInput value={classificationName} onChangeText={onClassificationNameChange} placeholder="例：牛乳" autoCapitalize="none" />
...
const styles = StyleSheet.create({
  ...
  scopeLabelSelected: { color: colors.primary }, scopeDetail: { color: colors.text.muted, fontSize: 12, marginTop: 2 },
  classificationNameField: { marginTop: spacing.sm }, fieldHint: { color: colors.text.muted, fontSize: 12, marginTop: spacing.xs },
});
