...
### 1.1 値引き等のカテゴリ配賦（Issue #114-9）

承認済みの負額の値引き・アプリ適用行は、明細保存時に世帯ごとの `isAdjustment = true` なCategory（標準名: `値引き等`）へ分類する。明細金額・レシート合計・ItemSplitは変更しない。

月次家計統計だけは、同一レシート内の正額かつ通常Categoryの明細金額を基準に、調整額をCategory別に比例配賦する。整数円への切捨てで残った額は、元の金額が最大のCategoryへ加算し、同額ならCategory IDの昇順で決める。配賦対象がない場合は、調整Categoryにそのまま残す。精算はItem金額を直接集計するため、この統計上の配賦では変化しない。
