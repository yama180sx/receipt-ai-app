...
> [!NOTE]
> Cloudflare R2への暗号化オフサイトコピーはIssue #57で段階導入中である。dev/stableは送信直後の実データ照合、起動後1回のtimer、失敗通知、読み戻し検証まで導入済みである。保持世代管理はIssue #57-3、隔離復旧演習は[Issue #57-4 runbook](../reviews/issue-57-4/r2-isolated-restore-runbook.md)で進める。R2への送信成功だけで復旧可能とみなさない。
