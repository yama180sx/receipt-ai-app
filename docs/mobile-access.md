...
| 症状 | 確認 |
|------|------|
| `:8082` の QR が `8081` になる / Expo Go が繋がらない | stable の `.env` に `EXPO_PACKAGER_PROXY_URL=http://HOST_IP:8082` があるか。`docker compose up -d frontend-dev` で再起動（compose は `DEV_PORT` から自動設定） |
