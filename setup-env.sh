...
if [ "$ENV" == "dev" ]; then
    ...
    EXPO_PORT_1=19000
    EXPO_PORT_2=19001
    # 【重要】QRコードのポートをホスト側に合わせる
    EXPO_PACKAGER_PROXY_URL="http://$T320_IP:$DEV_PORT"
    DB_PORT=5433
    REDIS_PORT=6380
    BACKEND_COMMAND="npm run dev"
    CORS_ORIGIN="http://localhost:$DEV_PORT,http://$T320_IP:$DEV_PORT,exp://$T320_IP:$DEV_PORT,http://$T320_IP:$WEB_PORT"
else
    ENV_NAME="stable"
    NODE_ENV="production"
    BACKEND_PORT=3000
    ...
    CORS_ORIGIN="http://$T320_IP:$WEB_PORT"
fi
...
EXPO_PUBLIC_API_URL="http://$T320_IP:$BACKEND_PORT/api"
...
echo "✅ Setup complete. Expo will now use $EXPO_PACKAGER_PROXY_URL for QR codes."